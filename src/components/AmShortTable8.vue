...
export default class AmShortTable8 extends Vue {
    ...
    private checkSubmit() {
        ...
        if (this.amshortfakedata.AmShort8FakeData === true) {
    ...
    private submitClick() {
        Axios.post(this.hosturl + 'SetAmShortTableData',
            {tablenumber: 8, usertype: this.usertype, datajson: JSON.stringify(this.localtable)})
        .then((res) => {
            console.log(res)
            const resobj = JSON.parse(res.data.d)
            if (resobj.Success === true) {
                this.amshorttable8 = resobj.NewData
                this.amshortfakedata.AmShort8FakeData = resobj.NewFakeData
                this.checkSubmit()
                this.myThis.$notify({
                    title: '提交成功',
                    message: '上午八表单数据提交成功',
